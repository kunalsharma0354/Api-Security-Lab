# NEXORA Â· API Security Lab

A professional, local-first educational dashboard for learning how common
API protection mechanisms behave â€” rate limiting, API key authentication,
input validation, request size limits, timeouts and layered defenses.

> **Status: All 7 lab modules + API key issuer live** â€” rate limiting, API key authentication,
...
| Malformed JSON â†’ structured validation error on the lab route | Done |
...
| GET | `/api/auth` | API-key protected API â€” requires `X-API-Key` header |
| POST | `/api/validate` | Strict input validation â€” rejects bad payloads with per-field errors |
| POST | `/api/payload` | Request size protection â€” oversized bodies get structured 413 |
| GET | `/api/timeout` | Timeout protection â€” slow work cut off with structured 504 |
...
toward `blockedRequests` â€” not toward errors. `/api/demo` is intentionally
unprotected for comparison.

API-key auth: `GET /api/auth` requires `X-API-Key`. Missing or invalid keys
get the same generic `401 { success:false, error:"Unauthorized" }` â€” no hint
...
exactly `{ name, email, age }`. Rules: name is required text of 2â€“100
...
Only byte counts are recorded â€” never body contents.
...
Copy `backend/.env.example` â†’ `backend/.env` and adjust:
...
public development value from `.env.example` â€” never a real secret â€” and is
...
â”œâ”€â”€ frontend/
â”‚   â”œâ”€â”€ components/     # Header, Sidebar, cards, table, toast, status panelâ€¦
â”‚   â”œâ”€â”€ pages/          # Dashboard, ApiLabs, RequestLogs, Analytics, ApiDocs, Settings
â”‚   â”œâ”€â”€ layouts/        # MainLayout (shell)
â”‚   â”œâ”€â”€ hooks/          # useServicesHealth (live /health polling), useLabRunner
â”‚   â”œâ”€â”€ styles/         # global.css design system
â”‚   â””â”€â”€ utils/          # constants, apiClient (typed fetch wrapper), format helpers
â”œâ”€â”€ backend/
â”‚   â”œâ”€â”€ src/
â”‚   â”‚   â”œâ”€â”€ server.ts   # bootstrap (+ fail-fast DEMO_API_KEY check), shutdown
â”‚   â”‚   â”œâ”€â”€ app.ts      # express app factory (cors, logging, routes, errors)
â”‚   â”‚   â”œâ”€â”€ config/     # env.ts (dotenv-backed)
â”‚   â”‚   â”œâ”€â”€ routes/     # health, demo, rate-limit, auth, metrics, logs
â”‚   â”‚   â”œâ”€â”€ middleware/ # requestLogger, errorHandler, rateLimiter, apiKeyAuth
â”‚   â”‚   â”œâ”€â”€ services/   # metricsService (in-memory records + snapshot math)
â”‚   â”‚   â””â”€â”€ types/      # shared response contracts
â”‚   â”œâ”€â”€ tests/api.test.ts
â”‚   â””â”€â”€ .env.example
â”œâ”€â”€ docs/               # architecture notes and roadmap
â”œâ”€â”€ tests/              # reserved for cross-cutting integration tests
â”œâ”€â”€ .env.example
â””â”€â”€ README.md
```
...
- **Part 1 (done)** â€” foundation, design system, full dashboard UI.
- **Part 2 (done)** â€” Express backend, health/demo/metrics/logs endpoints,
  request logging, live stats + status panel, Normal API wired end-to-end.
- **Part 3 (done)** â€” reusable fixed-window rate-limit middleware,
...
- **Part 4 (done)** â€” API-key authentication on `/api/auth`: reusable
...
- **Part 5 (done)** â€” strict input validation on `POST /api/validate`:
...
- **Part 6 (done)** â€” request size protection on `POST /api/payload`:
  ...
  by `/health`.
- **Part 7 (done)** â€” timeout protection on `GET /api/timeout`: deadline
...
- **Part 8 (done)** â€” multi-layer protection on `GET /api/protected`:
...
- **Future ideas** â€” persistence for logs/metrics, analytics charts,
  SQLite storage.
